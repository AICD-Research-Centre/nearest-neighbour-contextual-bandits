...
import os
import copy
from nncb.utils.bandit_class import BanditDataset

FILEPATH = "nncb/dataset_files/CICIDS2017/MachineLearningCVE"
...
class CICIDS2017(BanditDataset):
    def __init__(self, dataset_size=-1, num_features=77, relabel=False, Kurniabudi=False, use_weighting=False, distance_measure_model=None, port_index=0) -> None:
        ...
        self.distance_measure_model = distance_measure_model
    ...
    def load_in_data(self, num_features, dataset_size, relabel=False, get_df=False):
        ...
        self.feature_names = copy.deepcopy(df.columns)
    ...
    def load_in_one_day_of_data(self, day="Wednesday", num_features=77):
        # load in the data and remove redundant column, make sure action label is final column too
        # redundant column is duplicate of Fwd Header Length
        # df = pd.read_csv('data.csv')
        csv_files = [file for file in os.listdir(FILEPATH) if file.endswith('.csv')]
        ...
        dims_to_reduce = range(1, (self.dataset.shape[1] - 1)) # as 0 index is destination port, so don't want to change that as it's categorical?
        print(len(dims_to_reduce))
        self.dataset = self.reduce_data_range(self.dataset, dims_to_reduce=dims_to_reduce)
    ...
    def find_diff(self, x, y):
        if self.distance_measure_model is not None:
            x = np.array(copy.deepcopy(x), dtype=float)
            y = np.array(copy.deepcopy(y), dtype=float)
            x = x.reshape((1, x.shape[0]))
            y = y.reshape((1, y.shape[0]))
            diff = self.distance_measure_model.predict([x, y], verbose=0)
            return diff[0]
        
        processed_diff = x - y

        if self.port_index is not None and self.port_index < len(processed_diff):
            if x[self.port_index] == y[self.port_index]:
                # if port number is same
                processed_diff[self.port_index] = 0
            else:
                processed_diff[self.port_index] = 1
    ...
    def action_function_drift_test(self, x, a, label): 
