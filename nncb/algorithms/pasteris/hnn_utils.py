...
from nncb.algorithms.pasteris.cbnn import LeafList

VALUE = 'value'
DIFF = 'diff'
TRIAL_NUM = 'trial_num'

DEFAULT_LEAF_LIST_TYPE = None
...
class HSet:
    def __init__(self, depth, find_diff) -> None:
        self.d = depth
        self.trials = DEFAULT_LEAF_LIST_TYPE(find_diff)

class AllHSets:
    def __init__(self, binning_radius, rho, T, find_diff):#, dataset) -> None:
        self.HSets = {}
    ...
    def add_new_set(self, depth):
        if depth in self.HSets:
            ...
            self.HSets[depth] = HSet(depth, self.find_diff)
...
def hnn(model, H_sets, bt_B, bt_Z, tt_H_Z, leaf_list, x, label, trial_number):
    ...
    global DEFAULT_LEAF_LIST_TYPE

    if DEFAULT_LEAF_LIST_TYPE is None:
        DEFAULT_LEAF_LIST_TYPE = model.leaf_list_type
    ...
    if H_sets is None:
        ...
        H_sets = AllHSets(model.binning_radius, model.rho, model.T, model.find_diff)#, model.dataset)
        s_delta = 0
        delta = -1
        h = -1
    else:
